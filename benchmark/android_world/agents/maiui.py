...
def parse_tagged_text(text: str) -> Dict[str, Any]:
    ...
    if not match:
        raise ParseActionError(
            "MAI-UI response is missing the required thinking/tool_call tags."
        )
    raw_tool_call = match.group(2).strip().strip('"')
    return {
        "thinking": match.group(1).strip().strip('"'),
        "tool_call": mobile_agent_utils.parse_mobile_agent_tool_call(
            raw_tool_call
        ),
        "dummy_tool_call": raw_tool_call,
    }
...
class MAIUI(base_agent.EnvironmentInteractingAgent):
  ...
  def predict(
        ...
        if prediction is None:
            raise episode_exceptions.ModelEndpointError(
                "MAI-UI endpoint request failed after 3 attempts."
            ) from last_endpoint_error

        print(f"Raw response:\n{prediction}")
        # Parsing happens outside the endpoint retry block. Explicit schema
        # violations are model output; unexpected parser bugs propagate as
        # invalid infrastructure instead of being relabeled as endpoint errors.
  ...
  def step(
    ...
    try:
      ...
      if not dummy_tool_call or not dummy_tool_call.strip():
        raise ParseActionError("Empty tool call from MAI-UI response.")
