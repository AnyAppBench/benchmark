...
def _annotation_to_json_type(annotation: Any) -> str:
    """Best-effort conversion from Python annotations to JSON schema types."""
    if annotation is inspect.Signature.empty:
        return "string"
    origin = get_origin(annotation)
    if origin is Union:
        non_none_args = [arg for arg in get_args(annotation) if arg is not type(None)]
        return _annotation_to_json_type(non_none_args[0]) if non_none_args else "string"
...
def function_to_dict(func: Callable) -> Dict[str, Any]:
    """Create a LiteLLM/OpenAI-compatible tool schema from a Python function."""
