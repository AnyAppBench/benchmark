...
from android_world.agents.mobile_action_schema import (
    parse_mobile_agent_tool_call,
    validate_mobile_agent_action,
)
...
def convert_mobile_agent_action_to_json_action(
    ...
    dummy_action = validate_mobile_agent_action(dummy_action)
    ...
    if action_type == json_action.INPUT_TEXT:
      ...
      app_name = dummy_action_translated['arguments']['text']
        ...
        else:  # Defensive: validation above already rejects this branch.
            raise RuntimeError(
                f"Validated system button became unsupported: {arguments['button']!r}"
            )
