...
class M3AInteractionTest(absltest.TestCase):
  ...
  def test_step_method_with_llm_action_selection_error(self):
    env = test_utils.FakeAsyncEnv()
    llm = MockMultimodalLlmWrapper([(infer.ERROR_CALLING_LLM, None)])
    agent = m3a.M3A(env, llm)
...
class M3AUtilsTest(absltest.TestCase):

  def test_normalize_open_app_name_uses_goal_for_generic_clock(self):
    self.assertEqual(
        m3a_utils.normalize_open_app_name(
            'Clock', 'In the Google Clock app, run the stopwatch.'
        ),
        'google clock',
    )

  def test_normalize_open_app_name_keeps_specific_app_name(self):
    self.assertEqual(
        m3a_utils.normalize_open_app_name(
            'Fossify Messages', 'In the Fossify Messages app, send a text.'
        ),
        'fossify messages',
    )
