...
except ImportError:  # benchmark/ not on sys.path; plain goals still resolve.
...
try:
    ...
    from android_world.env import actuation as actuation_qwen3
...
class ParseActionError(episode_exceptions.ActionParseError):
    """Exception raised for errors in parsing an action response."""
...
class Qwen3_VL(base_agent.EnvironmentInteractingAgent):
  ...
  def __init__(self, env: interface.AsyncEnv, 
    ...
    if self.output_path:
      # Multiple app workers share the debug root during matrix execution.
      # Creating it must therefore be atomic and idempotent.
  ...
  def _task_output_dir_name(self, goal: str) -> str:
    """Resolves the debug-artifact directory name for ``goal``.

    ``task_name`` is keyed by the original ``task.goal`` while ``step`` may
    receive the augmented C2 prompt goal, so recover the original instruction
    before falling back to a slug; otherwise every C2 task collapses into one
    shared directory and overwrites the others' screenshots/action logs.
    """
    original_goal = goal
    if task_breakdowns_lib is not None:
      original_goal = task_breakdowns_lib.original_goal_from_prompt_goal(goal)
    name = self.task_name.get(goal)
    if name is None:
      name = self.task_name.get(original_goal)
    if name is None:
      name = original_goal.replace(" ", "_")[:50]
    return name
  ...
  def step(
    ...
    if self.output_path:
      ...
      task_output_dir = os.path.join(
          self.output_path, self._task_output_dir_name(goal)
      )
    ...
    result["prompt_system"] = system_prompt_part
    result["prompt_user"] = user_prompt_part
    ...
    try:
      if self.add_thought:
        ...
        if "Action:" in action_response:
          ...
          thought = action_response.split("<tool_call>", 1)[0].replace(
              "Thought:", ""
          ).strip()
          summary = ""
      ...
      if dummy_action is None:
        raise ParseActionError("No <tool_call> block found in model response.")
      dummy_action = mobile_agent_utils.parse_mobile_agent_tool_call(
          dummy_action
      )
    ...
    except episode_exceptions.DeclaredAgentOutputError:
      # Explicitly validated malformed model output is a scored agent failure.
      # It must escape to suite_utils so the typed attribution is retained.
      raise
    except Exception:  # pylint: disable=broad-exception-caught
      # Converter, evaluator, and runtime bugs are not model failures.
      traceback.print_exc()
      raise
    else:
      # VenusBench-Mobile
    ...
    if self.output_path:
      ...
      trace_record = {
          ...
          "prompt_system": system_prompt_part,
          "prompt_user": user_prompt_part,
          "response": action_response,
          "thought": thought,
          "action_desc": summary,
          "dummy_action": result.get("dummy_action"),
          "dummy_action_translated": result.get("dummy_action_translated"),
          "action": action,
      }
      trace_mode = 'w' if step_idx == 0 else 'a'
      with open(os.path.join(task_output_dir, "trace.jsonl"), trace_mode, encoding='utf-8') as f:
