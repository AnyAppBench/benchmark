...
_GENERIC_CLOCK_APP_NAMES = frozenset({'clock', 'clock app'})
...
def normalize_open_app_name(app_name: str, goal: str) -> str:
  """Normalizes generic app names using the task goal when it is unambiguous."""
  normalized = app_name.strip().lower()
  target_app = _target_app_from_goal(goal)
  if (
      normalized in _GENERIC_CLOCK_APP_NAMES
      and target_app.lower().endswith('clock')
  ):
