...
# This file is adapted from the upstream Mobile-Agent-v3 AndroidWorld
# integration (X-PLUG/MobileAgent, android_world_v3/.../mobile_agent_v3.py).
# Agent decision logic is preserved verbatim. The only CATBench-specific
# additions are evaluation-side trace capture (screenshot list, raw response
# list) and using `os.makedirs(..., exist_ok=True)` instead of `os.mkdir`
# so multiple emulator workers can safely share an output tree.
...
class MobileAgentV3_M3A(base_agent.EnvironmentInteractingAgent):
  ...
  def step(self, goal: str, step_numb: bool = False) -> base_agent.AgentInteractionResult:
    ...
    # CATBench eval-side trace capture: keep the per-step screenshots so the
    # checkpointer can serialise them along with InfoPool. Behaviourally inert.
    ...
    if not skip_manager:
      ...
      # CATBench eval-side trace capture.
