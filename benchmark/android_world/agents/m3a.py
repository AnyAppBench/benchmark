...
# Manually write prompts first and then try to use gpt to enhance this. Tell from execution, action. Send thoughts and action to gpt
...
Action: {{"action_type": "status", "goal_status": "infeasible"}}"""
    ...
    if not raw_response:
      print(
          'Error calling LLM in action selection phase. This should not happen: '
          f'{action_output}'
      )
      step_data['action_output'] = action_output
      step_data['action_raw_response'] = raw_response
      step_data['summary'] = (
          'Some error occurred calling LLM during action selection phase: %s'
          % action_output
      )
    ...
    try:
      ...
      if action_dict is None:
        raise ValueError('Could not extract JSON action.')
      if action_dict.get('action_type') == json_action.OPEN_APP:
        app_name = action_dict.get('app_name')
        if isinstance(app_name, str):
          action_dict['app_name'] = m3a_utils.normalize_open_app_name(
              app_name, goal
          )
