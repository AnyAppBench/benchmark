...
class AndroidWorldControllerTest(absltest.TestCase):
  ...
  @mock.patch.object(adb_utils, 'uiautomator_dump')
  @mock.patch.object(representation_utils, 'xml_dump_to_ui_elements')
  @mock.patch.object(android_world_controller, 'get_a11y_tree')
  def test_process_timestep_falls_back_to_uiautomator(
      self, mock_get_a11y_tree, mock_xml_to_ui, mock_uiautomator_dump
  ):
    mock_base_env = mock.Mock(spec=env_interface.AndroidEnvInterface)
    env = android_world_controller.AndroidWorldController(mock_base_env)
    mock_get_a11y_tree.side_effect = RuntimeError('a11y unavailable')
    mock_uiautomator_dump.return_value = '<hierarchy></hierarchy>'
    mock_ui_elements = mock.Mock()
    mock_xml_to_ui.return_value = mock_ui_elements
    timestep = dm_env.TimeStep(
        observation={}, reward=None, discount=None, step_type=None
    )
    ...
    mock_uiautomator_dump.assert_called_once_with(env._env)
    mock_xml_to_ui.assert_called_once_with('<hierarchy></hierarchy>')

  def test_refresh_a11y_forwarder_resends_grpc_port(self):
    mock_env = mock.Mock()
    mock_env.get_port.return_value = 8558

    android_world_controller._refresh_a11y_forwarder(mock_env)

    self.mock_issue_generic_request.assert_any_call(
        [
            'shell',
            'settings',
            'put',
  ...
  def test_get_a11y_forest_recovers_with_forwarder_refresh(
      self,
      mock_refresh_env,
      mock_has_wrapper,
      mock_get_controller,
    ...
    mock_refresh_env.assert_not_called()
