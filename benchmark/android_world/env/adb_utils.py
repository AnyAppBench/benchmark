...
_CATBENCH_EXTRA_APP_ALIASES: dict[str, tuple[str, ...]] = {
    'files_x_plore_file_manager': ('x-plore', 'xplore'),
    'sms_quik_sms': ('quik', 'quikr', 'quick sms'),
}
...
def _load_catbench_app_packages() -> dict[str, str]:
  """Returns display-name/app-id aliases for CATBench app package names."""
  csv_path = Path(__file__).resolve().parents[2] / 'app_generalization_apps.csv'
  out: dict[str, str] = {}

  def add_alias(alias: str, package_name: str) -> None:
    key = _normalize_app_lookup_key(alias)
    if key:
      out[key] = package_name
    ...
    with csv_path.open('r', encoding='utf-8', newline='') as handle:
      for row in csv.DictReader(handle):
        ...
        aliases = (
            ...
            row.get('app_id') or '',
            (row.get('app_id') or '').replace('_', ' '),
            package_name,
        )
        for alias in aliases:
          add_alias(alias, package_name)

        app_id = row.get('app_id') or ''
        if '_' in app_id:
          app_id_without_domain = app_id.split('_', 1)[1].replace('_', ' ')
          add_alias(app_id_without_domain, package_name)
          if app_id_without_domain.endswith(' contact'):
            add_alias(f'{app_id_without_domain}s', package_name)
        for alias in _CATBENCH_EXTRA_APP_ALIASES.get(app_id, ()):
          add_alias(alias, package_name)

        display_name = row.get('display_name') or ''
        if display_name.lower().endswith(' contact'):
          add_alias(f'{display_name}s', package_name)
  except OSError:
    logging.exception('Failed to load CATBench app package aliases: %s', csv_path)
  return out


_CATBENCH_APP_NAME_TO_PACKAGE = immutabledict.immutabledict(
    _load_catbench_app_packages()
)


def _catbench_package_for_app_name(app_name: str) -> str | None:
  return _CATBENCH_APP_NAME_TO_PACKAGE.get(_normalize_app_lookup_key(app_name))
...
def launch_app(
  ...
  package_name = _catbench_package_for_app_name(app_name)
  if package_name:
    response = issue_generic_request(
        ['shell', 'monkey', '-p', package_name, '1'], env, timeout_sec=5
    )
    logging.info(
        'Launching CATBench app %r by package name %r, response: %r',
        app_name,
        package_name,
        response,
    )
...
def clear_app_data(
  ...
  Returns:
    adb status.
  """
  timeout_sec = float(os.environ.get('CATBENCH_PM_CLEAR_TIMEOUT_SEC', 45))
  clear_method = os.environ.get('CATBENCH_CLEAR_APP_DATA_METHOD', 'pm_clear')
  clear_method = clear_method.strip().lower()
  if clear_method in ('root_rm', 'manual_root_rm'):
    pkg = shlex.quote(package_name)
    data_dir = shlex.quote(f'/data/data/{package_name}')
    user_de_dir = shlex.quote(f'/data/user_de/0/{package_name}')
    external_cache = shlex.quote(
        f'/sdcard/Android/data/{package_name}/cache'
    )
    script = (
        f'am force-stop {pkg}; '
        f'for dir in {data_dir} {user_de_dir}; do '
        'if [ -d "$dir" ]; then '
        'find "$dir" -mindepth 1 -maxdepth 1 ! -name lib '
        '-exec rm -rf -- {} +; '
        'fi; '
        'done; '
        f'rm -rf -- {external_cache}'
    )
    return issue_generic_request(['shell', script], env, timeout_sec=timeout_sec)
  if clear_method in ('force_stop', 'force_stop_only'):
    return issue_generic_request(
  ...
  try:
    return issue_generic_request(
        ['shell', 'pm', 'clear', package_name], env, timeout_sec=timeout_sec
    )
...
def uiautomator_dump(env, timeout_sec: Optional[float] = 30) -> str:
  ...
  attempts = (
      ...
      (
          'uiautomator --compressed',
          [
              'shell',
              'uiautomator',
              'dump',
              '--compressed',
              '/sdcard/window_dump_compressed.xml',
          ],
          ['shell', 'cat', '/sdcard/window_dump_compressed.xml'],
      ),
  )
  errors_seen = []
  for label, dump_args, read_args in attempts:
    dump_response = issue_generic_request(
        dump_args, env, timeout_sec=timeout_sec
    )
    dump_output = dump_response.generic.output.decode(
        'utf-8', errors='replace'
    )
    if (
        dump_response.status != adb_pb2.AdbResponse.Status.OK
        or 'ERROR:' in dump_output
    ):
      errors_seen.append(f'{label}: {dump_output.strip()}')
      continue

    read_response = issue_generic_request(
        read_args, env, timeout_sec=timeout_sec
    )
    xml = read_response.generic.output.decode('utf-8', errors='replace')
    if (
        read_response.status == adb_pb2.AdbResponse.Status.OK
        and '<hierarchy' in xml
    ):
      if label != 'uiautomator':
        logging.warning('Using compressed uiautomator dump fallback.')
      return xml
    errors_seen.append(f'{label}: invalid dump XML')

  raise RuntimeError(
      'Could not retrieve UI hierarchy via uiautomator. '
      + ' | '.join(errors_seen)
  )
