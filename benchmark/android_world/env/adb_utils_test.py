...
class AdbSettingsTest(AdbTestSetup):
  ...
  def test_launch_catbench_display_name_uses_package_name(self):
    adb_utils.launch_app('Fossify Messages', self.mock_env)

    self.mock_issue_generic_request.assert_called_once_with(
        ['shell', 'monkey', '-p', 'org.fossify.messages', '1'],
        self.mock_env,
        timeout_sec=5,
    )

  def test_launch_catbench_clock_overrides_generic_clock_mapping(self):
    adb_utils.launch_app('Clock', self.mock_env)

    self.mock_issue_generic_request.assert_called_once_with(
        ['shell', 'monkey', '-p', 'com.best.deskclock', '1'],
        self.mock_env,
        timeout_sec=5,
    )

  def test_launch_catbench_plural_generated_contact_name_uses_package_name(self):
    adb_utils.launch_app('right contacts', self.mock_env)

    self.mock_issue_generic_request.assert_called_once_with(
        ['shell', 'monkey', '-p', 'com.goodwy.contacts', '1'],
        self.mock_env,
        timeout_sec=5,
    )

  def test_launch_catbench_quik_sms_common_alias_uses_package_name(self):
    adb_utils.launch_app('quikr', self.mock_env)

    self.mock_issue_generic_request.assert_called_once_with(
        ['shell', 'monkey', '-p', 'dev.octoshrimpy.quik.fdroid', '1'],
        self.mock_env,
        timeout_sec=5,
    )

  def test_launch_catbench_xplore_short_alias_uses_package_name(self):
    adb_utils.launch_app('X-plore', self.mock_env)

    self.mock_issue_generic_request.assert_called_once_with(
        ['shell', 'monkey', '-p', 'com.lonelycatgames.Xplore', '1'],
        self.mock_env,
        timeout_sec=5,
    )
