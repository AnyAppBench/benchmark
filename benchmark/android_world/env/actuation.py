...
def execute_adb_action(
  ...
  if action.action_type in ['click', 'double_tap', 'long_press']:
    ...
    if isinstance(direction, (list, tuple)) and len(direction) == 4:
      ...
      end_x_attr = getattr(action, 'x_', None)
      end_y_attr = getattr(action, 'y_', None)
      if end_x_attr is not None and end_y_attr is not None:
        start_x, start_y = action.x, action.y
        end_x, end_y = end_x_attr, end_y_attr
      else:
