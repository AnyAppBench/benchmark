...
class EpisodeRunnerTest(absltest.TestCase):
  ...
  def test_health_check_runs_after_reset_and_each_action(self):
    mock_agent = FakeEnvironmentInteractingAgent(
        self.env, 'fake_agent', return_done=True
    )
    health_check = mock.MagicMock()

    result = episode_runner.run_episode(
        'test_goal', mock_agent, health_check_fn=health_check
    )

    self.assertTrue(result.done)
    self.assertEqual(
        [mock.call(self.env), mock.call(self.env)],
        health_check.call_args_list,
    )

  def test_post_action_health_error_propagates_before_done(self):
    mock_agent = FakeEnvironmentInteractingAgent(
        self.env, 'fake_agent', return_done=True
    )
    health_check = mock.MagicMock(
  ...
  def test_zero_step_episode_still_checks_runtime_health(self):
    mock_agent = FakeEnvironmentInteractingAgent(self.env, 'fake_agent')
    health_check = mock.MagicMock()
    ...
    self.assertFalse(result.done)
    health_check.assert_called_once_with(self.env)
    self.assertEqual(mock_agent.call_count, 0)
